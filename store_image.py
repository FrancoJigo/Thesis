...
db_conn_str = "dbname=copepod_image user=postgres"
...
def main(argv):
    ...
    if args.store:
        ...
        #
        filedata = f.read()
        curs.execute("INSERT INTO species_1(species_1_id, species_1_name, image) VALUES (DEFAULT,%s,%s) RETURNING species_1_id", (args.filename, filedata))
        ...
        curs.execute("SELECT image, species_1_name FROM species_1 WHERE species_1_id = %s", (int(args.fetch),))
        (file_data, species_1_name) = curs.fetchone()

            # In Python 3 this code works as-is.
            # In Python 2, you must get the str from the returned buffer object.
        f.write(file_data)
        f.close()
        print("Fetched {0} into file {1}; species name was {2}".format(args.fetch, args.filename, species_1_name))
